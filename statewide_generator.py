...
output_file = election+'__ks__general__precinct.csv'
...
def generate_consolidated_file(year, path, output_file):
    ...
    for fname in glob.glob(path):
        with open(fname, "r") as csvfile:
            ...
            reader = csv.DictReader(csvfile)
            for row in reader:
                if row['office'].strip() in ['President', 'Governor', 'Secretary of State', 'State Auditor', 'State Treasurer', 'Commissioner of Agriculture & Commerce', 'Commissioner of Insurance', 'Attorney General', 'U.S. House', 'State Senate', 'State House', 'U.S. Senate']:
                    results.append([row['county'], row['precinct'], row['office'], row['district'], row['candidate'], row['party'], row['votes'], row['vtd']])
    os.chdir('..')
    os.chdir('..')
